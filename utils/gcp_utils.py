...
def params() -> argparse.Namespace:
    ...
    parser.add_argument(
        "action", type=str, choices=TUP_TIPO_ACCION, help="Tipo de acción a realizar"
    )
    ...
    if args.action in [TUP_TIPO_ACCION[0], TUP_TIPO_ACCION[2]] and not args.bucket_name:
        parser.error(
            f"El argumento 'bucket_name' es obligatorio cuando la acción es '{args.action}'"
        )

    return args


def get_project_id_from_credentials(project_id: str | None = None) -> str | None:
...
def setup_gcp_credentials() -> bool:
    ...
    if os.getenv("GOOGLE_APPLICATION_CREDENTIALS"):
        print(
            f"✅ Variable GOOGLE_APPLICATION_CREDENTIALS ya está configurada: {os.getenv('GOOGLE_APPLICATION_CREDENTIALS')}"
        )
        return True
            ...
            if json_files:
                ...
                if len(json_files) == 1:
                    ...
                    service_account_files = [
                        f for f in json_files if "service-account" in f.name.lower()
                    ]
...
def create_bucket(
    bucket_name: str, location: str = "southamerica-east1", project_id: str | None = None
) -> bool:
...
def list_buckets(project_id: str | None = None) -> bool:
...
def delete_bucket(bucket_name: str, project_id: str | None = None) -> bool:
...
def main() -> None:
    ...
    if args.action == TUP_TIPO_ACCION[0]:  # gcs-create
        ...
        success = create_bucket(
            args.bucket_name, location=args.location, project_id=args.project_id
        )
