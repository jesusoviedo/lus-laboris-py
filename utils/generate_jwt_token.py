...
class JWTTokenGenerator:
    ...
    def __init__(self):
        ...
        self.private_key_path = (
            private_path
            if os.path.isabs(private_path)
            else os.path.join(self.project_root, private_path)
        )
        self.public_key_path = (
            public_path
            if os.path.isabs(public_path)
            else os.path.join(self.project_root, public_path)
        )
    ...
    def generate_token(
        ...
        try:
            ...
            print_info(
                f"Token expira en: {expiry.strftime('%Y-%m-%d %H:%M:%S UTC')} ({expiry_minutes} minutos)"
            )
            return token
        except Exception as e:
...
def main():
    ...
    parser.add_argument("--username", type=str, help="Nombre de usuario para el token")

    parser.add_argument(
        "--expiry", type=int, help="Expiración del token en minutos (por defecto: 15)"
    )
    ...
    parser.add_argument(
        "--output", type=str, help="Archivo de salida para guardar el token (opcional)"
    )

    # Token validation arguments
    parser.add_argument("--validate", action="store_true", help="Validar un token existente")
    parser.add_argument("--token", type=str, help="Token a validar (requerido con --validate)")
    ...
    try:
        token = generator.generate_token(
            username=args.username, expiry_minutes=args.expiry, additional_claims=additional_claims
        )
