...
class GCPService:
        ...
        try:
            ...
            if os.getenv("K_SERVICE"):  # Cloud Run environment variable
                ...
                if not self.credentials_path:
                    logger.warning("GOOGLE_APPLICATION_CREDENTIALS not set - GCP features will be unavailable")
                    return False
                
                if not os.path.exists(self.credentials_path):
                    logger.warning(f"Credentials file not found: {self.credentials_path} - GCP features will be unavailable")
                    return False
            ...
            logger.warning(f"Failed to initialize GCS client: {str(e)} - GCP features will be unavailable")
            return False
    ...
    def load_from_gcs_local(
        ...
        try:
            ...
            if use_credentials:
                ...
                # Initialize client if not already done
                if not self._initialize_client():
                    raise ConnectionError("GCP client not initialized. Please configure GOOGLE_APPLICATION_CREDENTIALS")
                client = self.client
            ...
            logger.info(f"Successfully loaded JSON file with {len(data.get('articulos', []))} articles")
            
            return data
            
        except Exception as e:
            logger.error(f"Failed to load JSON from GCS (local mode): {str(e)}")
            raise
    
    def load_from_gcs_cloud(
        self,
        filename: str,
        folder: str,
        bucket_name: str
        ...
        try:
            ...
            logger.error(f"Failed to load JSON from GCS (cloud mode): {str(e)}")
            raise
    ...
    def load_json_from_gcs(
        ...
        try:
            ...
            logger.error(f"Failed to load JSON from GCS: {str(e)}")
            raise
    ...
    def list_files_in_bucket(
        ...
        try:
            ...
            logger.info(f"Found {len(file_paths)} files in bucket {bucket_name} with prefix '{prefix}'")
            return file_paths
