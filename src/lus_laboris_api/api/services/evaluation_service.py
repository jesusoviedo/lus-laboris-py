...
class EvaluationService:
    ...
    async def _run_evaluation_async(self, eval_task: dict[str, Any]):
        ...
        try:
            ...
            # Extract results
            relevance_score = (
                evaluation_results[0] if not isinstance(evaluation_results[0], Exception) else None
            )
            hallucination_score = (
                evaluation_results[1] if not isinstance(evaluation_results[1], Exception) else None
            )
            toxicity_score = (
                evaluation_results[2] if not isinstance(evaluation_results[2], Exception) else None
            )
    ...
    def _evaluate_hallucination(self, question: str, context: str, answer: str) -> float | None:
        ...
        try:
            ...
            if eval_response and (
                "hallucinated" in eval_response.lower() or "hallucination" in eval_response.lower()
            ):
    ...
    def _evaluate_toxicity(self, answer: str) -> float | None:
        ...
        try:
            ...
            if eval_response and (
                "no-tóxico" in eval_response.lower()
                or "no toxico" in eval_response.lower()
                or "no tóxico" in eval_response.lower()
            ):
                return 0.0  # Non-toxic
            if eval_response and (
                "tóxico" in eval_response.lower() or "toxico" in eval_response.lower()
            ):
                return 1.0  # Toxic
    ...
    def _calculate_overall_quality(
        self, relevance: float | None, hallucination: float | None, toxicity: float | None
    ) -> float | None:
        ...
        try:
            ...
            logger.exception("Failed to calculate overall quality")
            return None

    def _save_evaluation_to_phoenix(
        self, session_id: str, metrics: dict[str, Any], eval_task: dict[str, Any]
    ):
