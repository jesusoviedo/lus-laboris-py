    ...
    def health_check(self) -> dict[str, Any]:
        ...
        try:
            if not self.use_reranking:
                return {"status": "disabled", "message": "Reranking is disabled in configuration"}

            if not self.model_name:
                return {"status": "unhealthy", "error": "No reranking model configured"}

            if not self.model:
                return {"status": "unhealthy", "error": "Reranking model not loaded"}
