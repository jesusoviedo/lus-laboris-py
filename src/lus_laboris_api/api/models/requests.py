...
class LoadToVectorstoreLocalRequest(BaseModel):
    ...
    filename: str = Field(
        ..., 
        description="Name of the JSON file to load (without path)",
        min_length=1
    )
    ...
    replace_collection: bool = Field(
        False,
        description="Whether to replace existing collection if it exists"
    )
...
class LoadToVectorstoreGCPRequest(BaseModel):
    ...
    folder: str = Field(
        ..., 
        description="Folder path in the GCS bucket where the file is located",
        min_length=1
    )
...
class LoadToVectorstoreRequest(BaseModel):
    ...
    collection_name: Optional[str] = Field(
        None,
        description="Name of the Qdrant collection. If not provided, will be auto-generated"
    )
    ...
    bucket_name: Optional[str] = Field(
        None,
        description="Name of the GCS bucket (required for GCP mode)"
    )
    
    gcp_credentials_path: Optional[str] = Field(
        None,
        description="Path to GCP credentials JSON file (required when running locally)"
    )
    ...
    embedding_model: Optional[str] = Field(
        None,
        description="Name of the embedding model to use. If not provided, will use default"
    )
