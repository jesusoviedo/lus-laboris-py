...
from ..auth.security import get_current_user, require_vectorstore_write
from ..config import settings
from ..models.requests import LoadToVectorstoreGCPRequest, LoadToVectorstoreLocalRequest
...
def _timestamp_to_datetime(timestamp: float | None) -> str | None:
...
async def _load_to_vectorstore_background(
    ...
    try:
        ...
        documents, embeddings, _embedding_metadata = await _process_data_for_embedding(
...
async def get_collection_info(collection_name: str, current_user: str = Depends(get_current_user)):
...
async def delete_collection(
    request: Request, collection_name: str, current_user: str = Depends(require_vectorstore_write)
):
    """Delete a collection"""
    session_id = None
    try:
        ...
        phoenix_service.track_vectorstore_operation(
            ...
            collection_name=collection_name,
            metadata={"success": True, "collection_deleted": collection_name},
        )

        return CollectionDeleteResponse(
            success=True, message=f"Collection '{collection_name}' deleted successfully"
        )
...
async def _process_data_for_embedding(
    data: dict[str, Any], model_name: str, batch_size: int
) -> tuple:
    ...
    try:
        ...
        embeddings, embedding_metadata = embedding_service.generate_embeddings(
            texts=texts, model_name=model_name, batch_size=batch_size
        )
...
async def list_all_jobs(current_user: str = Depends(require_vectorstore_write)):
    """List all background jobs (accessible by any authenticated user)"""
    try:
        all_jobs = []

        for _job_id, job_data in background_jobs.items():
