...
async def health_check():
    ...
    try:
        ...
        # Extract results
        qdrant_status = (
            health_check_tasks[0]
            if not isinstance(health_check_tasks[0], Exception)
            else {"status": "error"}
        )
        gcp_status = (
            health_check_tasks[1]
            if not isinstance(health_check_tasks[1], Exception)
            else {"status": "error"}
        )
        embedding_status = (
            health_check_tasks[2]
            if not isinstance(health_check_tasks[2], Exception)
            else {"status": "error"}
        )
        reranking_status = (
            health_check_tasks[3]
            if not isinstance(health_check_tasks[3], Exception)
            else {"status": "error"}
        )
        rag_status = (
            health_check_tasks[4]
            if not isinstance(health_check_tasks[4], Exception)
            else {"status": "error"}
        )
        eval_status = (
            health_check_tasks[5]
            if not isinstance(health_check_tasks[5], Exception)
            else {"status": "error"}
        )
        phoenix_status = (
            health_check_tasks[6]
            if not isinstance(health_check_tasks[6], Exception)
            else {"status": "error"}
        )
        ...
        all_healthy = (
            ...
            and rag_status.get("status") == "healthy"
        )
...
async def qdrant_health_check(token_payload: dict[str, Any] = Depends(optional_auth)):
...
async def gcp_health_check(token_payload: dict[str, Any] = Depends(optional_auth)):
...
async def embedding_health_check(token_payload: dict[str, Any] = Depends(optional_auth)):
    ...
    try:
        ...
        return {
            ...
            else "Embedding service has issues",
            "status": sanitized_status,
            "timestamp": time.time(),
        }
...
async def reranking_health_check(token_payload: dict[str, Any] = Depends(optional_auth)):
    ...
    try:
        ...
        return {
            "success": status.get("status") == "healthy",
            "message": "Reranking service is healthy"
            if status.get("status") == "healthy"
            else "Reranking service has issues",
            "status": sanitized_status,
            "timestamp": time.time(),
        }
...
async def rag_health_check(token_payload: dict[str, Any] = Depends(optional_auth)):
...
async def phoenix_health_check(token_payload: dict[str, Any] = Depends(optional_auth)):
    """
    ...
    **Optional authentication** with smart information filtering:
    ...
    try:
        ...
        return {
            ...
            "message": "Phoenix monitoring is healthy"
            if status.get("status") == "healthy"
            else "Phoenix monitoring has issues",
            "status": sanitized_status,
            "timestamp": time.time(),
        }
